...
FORMATS={"image":{"jpg","jpeg","png","webp"},"video":{"mp4","mov","avi","webm"},"audio":{"wav","flac","mp3","ogg","m4a"}}
...
def model_status(): return jsonify({**detector.status(),"supported_formats":{k:sorted(v) for k,v in FORMATS.items()},"max_upload_mb":50})
...
def upload():
    ...
    if kind is None:
        supported=sorted({ext for values in FORMATS.values() for ext in values})
        return jsonify({"error":f"Unsupported format. Use: {', '.join(supported)}."}),415
...
def too_large(_): return jsonify({"error":"File is larger than the 50 MB limit."}),413
